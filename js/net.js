...
var Net = (function () {
  ...
  function buildAvatar(color) {
    ...
    var body = new THREE.MeshStandardMaterial({ color: color, roughness: 0.85, flatShading: true });
    var dark = new THREE.MeshStandardMaterial({ color: 0x20261a, roughness: 1, flatShading: true });
    var torso = new THREE.Mesh(new THREE.BoxGeometry(0.9, 1.1, 0.5), body); torso.position.y = 1.5; g.add(torso);
    var head = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.6, 0.6), body); head.position.y = 2.35; g.add(head);
    ...
    var lm = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.95, 0.3), dark); lm.position.y = -0.47; legL.add(lm); g.add(legL);
    var legR = new THREE.Group(); legR.position.set(0.24, 0.95, 0);
    var rm = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.95, 0.3), dark); rm.position.y = -0.47; legR.add(rm); g.add(legR);
    var armL = new THREE.Group(); armL.position.set(-0.62, 2.0, 0);
    var alm = new THREE.Mesh(new THREE.BoxGeometry(0.26, 1.0, 0.26), body); alm.position.y = -0.5; armL.add(alm); g.add(armL);
    var armR = new THREE.Group(); armR.position.set(0.62, 2.0, 0);
    var arm2 = new THREE.Mesh(new THREE.BoxGeometry(0.26, 1.0, 0.26), body); arm2.position.y = -0.5; armR.add(arm2); g.add(armR);
